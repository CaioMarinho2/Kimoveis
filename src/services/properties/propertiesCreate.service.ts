...
async function propertiesCreateService( {value,size,address,categoryId}:IPropertyRequest) {
    if(!value||!size||!address.district||!address.zipCode||!address.city||!address.state||!categoryId){
    ...
    const categoryIdValid=categories.find((categories)=>categories.id===categoryId)
    const adressesValidZipCode=addresses.find((addresses)=>addresses.zipCode===address.zipCode)
    const adressesValidNumber=addresses.find((addresses)=>addresses.number===address.number)
